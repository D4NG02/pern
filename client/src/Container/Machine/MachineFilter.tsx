...
import { Box, Button, FormControl, FormLabel, MenuItem, Select, Stack, Typography } from '@mui/material';
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <DemoContainer components={['DatePicker']} sx={{ paddingTop: 'unset' }}>
            <DatePicker sx={{  minWidth: '160px', '.MuiInputBase': { '&-root': { color: 'white', bgcolor: constantStyle.color_primary }},
                                      ...
                                      'button': { color: 'white' },
                                      'svg': { fontSize: '16px' } }} views={['year', 'month', 'day']}
                                value={value} onChange={(newValue) => {
                                                        setValue(newValue)
                                                        let day = newValue?.date()
                                                        let month = newValue?.month()? newValue?.month()+1:0
